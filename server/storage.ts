...
conditions.push(eq(serviceCategories.slug, categorySlug));
    ...
    // 💥 FIX START: Only apply location filter if all required params are valid 💥
    // Check if lat/lng are provided AND radius is a positive number
    const isValidLocationFilter = 
      latitude !== undefined && 
      longitude !== undefined && 
      radius && 
      radius > 0;
...
const [provider] = await db.update(serviceProviders).set({ rating: newRating.toFixed(2), reviewCount }).where(eq(serviceProviders.id, providerId)).returning();
return provider;
}
