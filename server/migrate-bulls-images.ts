...
async function migrateExternalImages() {
    console.log("🔍 Finding external image URLs for 'Bulls & Trader cafe'...");
    
    // Find Bulls & Trader cafe
    const providers = await db.query.serviceProviders.findMany({
        where: (sp, { ilike }) => ilike(sp.businessName, '%Bulls%')
    });
    
    if (providers.length === 0) {
        console.error("❌ Bulls & Trader cafe not found!");
        process.exit(1);
    }
    ...
    // Find items with non-Cloudinary external URLs (Freepik, etc.)
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        try {
            ...
            const result = await cloudinary.uploader.upload(item.imageUrl!, {
                folder: 'shirur-express/bulls-cafe',
                resource_type: 'image',
                transformation: [
