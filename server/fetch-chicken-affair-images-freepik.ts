...
function generateSearchQuery(name: string, category: string): string {
    ...
    if (category === 'Chicken Main Course') {
        return `${query} chicken curry indian dish`;
    }
    if (category === 'Mutton Main Course') {
        return `${query} mutton curry indian dish`;
    }
    if (category === 'Tandoori Starters' || category.includes('Starter')) {
        return `${query} indian tandoor starter nonveg`;
    }
    if (category === 'Biryani' || category === 'Rice') {
        return `${query} indian rice biryani dish`;
    }
    if (category === 'Breads' || category === 'Roti') {
        return `${query} indian flatbread roti`;
    }

    // Default: add "indian meat dish" for relevance if it's chicken affair
    return `${query} indian non veg meat food`;
}

// ===== Main execution =====
async function fetchChickenAffairImages() {
    console.log("🔍 Looking for 'Chicken Affair' provider...\n");

    const provider = await db.query.serviceProviders.findFirst({
        where: eq(serviceProviders.businessName, "Chicken Affair"), // Exact match based on log
    });

    if (!provider) {
        console.error("❌ 'Chicken Affair' provider not found in database!");
        process.exit(1);
    }

    console.log(`✅ Found: ${provider.businessName} (ID: ${provider.id})`);

    // Fetch all menu items for this provider without image or with empty image
    ...
    console.log(`   ⏭️ Skipped: ${skipped}/${items.length}`);
...
fetchChickenAffairImages().catch((err) => {
